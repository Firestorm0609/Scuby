...
from telethon.errors import SessionPasswordNeededError, PhoneCodeInvalidError, FloodWaitError, PersistentTimestampOutdatedError
...
class SessionManager:
    ...
    async def setup_monitoring(self, user_id: int, client: TelegramClient):
        ...
        Wraps the event handler to automatically recover from
        PersistentTimestampOutdatedError and similar connection-level errors
        by disconnecting, reconnecting, and re-subscribing.
        """
        reconnect_counts: Dict[int, int] = getattr(self, '_reconnect_counts', {})
        self._reconnect_counts = reconnect_counts
        ...
        async def user_message_handler(event):
            ...
            try:
                ...
                logger.error(
                    f"⚠️ PersistentTimestampOutdatedError for user {user_id} "
                    f"— triggering auto-reconnect"
                )
                await self._reconnect_user(user_id)
            except AuthKeyUnregisteredError:
                logger.error(
                    f"🔑 AuthKeyUnregisteredError for user {user_id} "
                    f"— session invalidated, marking inactive"
                )
                self.db.mark_session_inactive(user_id)
            except Exception as e:
                logger.error(
                    f"❌ Error in message handler for user {user_id}: {e}",
                    exc_info=True,
                )
        
        print(f"✅ Monitoring setup for user {user_id}")
    
    async def _reconnect_user(self, user_id: int, max_retries: int = 3) -> bool:
        """Disconnect and reconnect a user's Telethon client.
        
        Handles PersistentTimestampOutdatedError by forcing a fresh
        connection which resets the persistent timestamp state.
        Uses exponential backoff between retries.
        """
        reconnect_counts = getattr(self, '_reconnect_counts', {})
        attempts = reconnect_counts.get(user_id, 0)

        if attempts >= max_retries:
            logger.error(
                f"❌ Max reconnect attempts ({max_retries}) reached for user {user_id}. "
                f"Marking session inactive."
            )
            self.db.mark_session_inactive(user_id)
            reconnect_counts[user_id] = 0
            # Disconnect and remove the client
            await self.disconnect_user(user_id)
            return False

        reconnect_counts[user_id] = attempts + 1
        self._reconnect_counts = reconnect_counts

        delay = min(2 ** attempts * 5, 120)  # 5s, 10s, 20s, max 120s
        logger.info(
            f"🔄 Reconnecting user {user_id} in {delay}s "
        ...
        if not client:
            # Client was removed; try loading from DB
            success, msg = await self.load_user_session(user_id)
            if success:
                reconnect_counts[user_id] = 0
            return success
        ...
        try:
            ...
            if not await client.is_user_authorized():
                logger.error(f"🔑 User {user_id} session no longer authorized after reconnect")
                self.db.mark_session_inactive(user_id)
                del self.user_clients[user_id]
                reconnect_counts[user_id] = 0
                return False

            # Re-setup monitoring with a fresh event handler
            await self.setup_monitoring(user_id, client)
            reconnect_counts[user_id] = 0  # Reset on success
            logger.info(f"✅ Successfully reconnected user {user_id}")
            return True

        except Exception as e:
            logger.error(
                f"❌ Reconnect failed for user {user_id}: {e}", exc_info=True
            )
            return await self._reconnect_user(user_id, max_retries=max_retries)
    ...
    async def test_chat_access(self, user_id: int, chat_id: int, check_send: bool = False) -> tuple[bool, str]:
        """Test if user has access to a chat.
        
        Args:
            check_send: If True, also verifies the client can send messages
                        to this chat (rejects bots, checks channel posting rights).
        """
        client = self.get_user_client(user_id)
        if not client:
        ...
        try:
            ...
            # Additional checks for target chats
            if check_send:
                from telethon.tl.types import User, Channel
                
                # Reject bot accounts — bots cannot message other bots
                if isinstance(entity, User) and entity.bot:
                    return (False, "❌ Cannot send messages to a bot account. Telegram bots cannot message other bots. Please use a group or channel as the target.")
                
                # For broadcast channels, verify the user has posting rights
                if isinstance(entity, Channel) and entity.broadcast:
                    try:
                        participant = await client.get_participant(entity)
                        # Check if user can post
                        if hasattr(participant, 'admin_rights') and participant.admin_rights:
                            if not participant.admin_rights.publish_messages:
                                return (False, f"❌ You don't have posting rights in '{name}'. Ask a channel admin to grant you posting permissions.")
                        elif not isinstance(participant, (type(None),)):
                            # User is a member but not an admin — can't post
                            return (False, f"❌ You're not an admin in channel '{name}'. Only channel admins can post messages there.")
                    except Exception:
                        # If we can't check permissions, warn but don't block
                        pass
