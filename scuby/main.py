...
from proactive import rug_watchdog_job, whale_alert_job
from memory import (
...
async def post_init(application: Application) -> None:
    ...
    # Load persistent session memory (survives bot restarts)
    try:
        from codebase import load_session, _load_conversations
        session = load_session()
        convos = _load_conversations()
        logger.info(f"Loaded persistent session: {len(session)} entries, {len(convos)} user conversations")
    ...
    jq.run_daily(daily_briefing_job,         time=dt_time(9, 0, 0))
...
async def post_shutdown(application: Application) -> None:
    ...
    try:
        ...
        pass
    # Save persistent session memory
    try:
        from codebase import save_session
        save_session({"shutdown_ts": __import__('time').time()})
    except Exception:
        pass
...
def main():
    ...
    app.add_handler(CommandHandler("selftest",       handle_selftest))
    app.add_handler(CommandHandler("read",           handle_read))
    app.add_handler(CommandHandler("status",         handle_status))
    app.add_handler(CommandHandler("fix",            handle_fix))
    app.add_handler(CommandHandler("fixyes",         handle_fix_confirm))
    app.add_handler(CommandHandler("searchcode",     handle_search_code))
    ...
    app.add_handler(CommandHandler("packages",       handle_packages))

    # ── Health & Docs ─────────────────────────────────────────────────────────
    app.add_handler(CommandHandler("health",         handle_health))
    app.add_handler(CommandHandler("docs",           handle_docs))
    ...
    # ── Analysis ──────────────────────────────────────────────────────────────
    app.add_handler(CommandHandler("deps",           handle_deps))
    app.add_handler(CommandHandler("deadcode",       handle_deadcode))
    app.add_handler(CommandHandler("envcheck",       handle_envcheck))
    app.add_handler(CommandHandler("configcheck",    handle_configcheck))
    app.add_handler(CommandHandler("changelog",      handle_changelog))
    app.add_handler(CommandHandler("coverage",       handle_coverage))
    app.add_handler(CommandHandler("typecheck",      handle_typecheck))

    # ── Codebase intelligence ─────────────────────────────────────────────────
    app.add_handler(CommandHandler("refactor",       handle_refactor))
    app.add_handler(CommandHandler("context",        handle_context))
    app.add_handler(CommandHandler("trace",          handle_trace))
