...
from memory import (
    analyse_patterns, _bucket,
    MCAP_BUCKETS, LIQ_BUCKETS, AGE_BUCKETS, VOLMCAP_BUCKETS,
)
...
def format_gem_score(result: dict, symbol: str, ca: str) -> str:
    ...
    if score >= 75:
        tip = "Strong early signal — watch closely."
    elif score >= 55:
        tip = "Decent setup — standard risk."
    elif score >= 35:
        tip = "Notable red flags — proceed with caution."
    else:
        tip = "Multiple danger signs — be very careful."

    lines.append(f"\n_{e(tip)}_")
    lines.append(f"_GemScore is informational only._")
    return "\n".join(lines)
