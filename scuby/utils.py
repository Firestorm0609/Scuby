...
def is_owner(user_id: int | None) -> bool:
    """Check if a user_id matches the bot owner."""
...
VALID_EVM_CA      = re.compile(r'^0x[0-9a-fA-F]{40}$')
VALID_TICKER      = re.compile(r'^[A-Z]{2,10}$')

# Chain detection from address format
def detect_chain(address: str) -> str:
    """Detect blockchain from address format. Returns 'solana', 'evm', or 'unknown'."""
    if VALID_CA.match(address):
        return "solana"
    if VALID_EVM_CA.match(address):
        return "evm"
    return "unknown"
...
async def get_all_by_ca(ca: str, http: httpx.AsyncClient) -> list[dict]:
    ...
    if chain == "evm":
        # EVM address — return all chains (DexScreener supports multi-chain)
        pairs = [p for p in data.get("pairs", []) or []]
    else:
        # Solana address (or unknown) — filter to Solana only
    ...
    if not symbol:
        return pairs
    # Only search all versions for Solana tokens (EVM tokens don't have OG concept)
    if chain == "solana":
...
def format_og_response(og: dict, total_versions: int, risk_badge: str, scan_prices: dict) -> str:
    ...
    versions_line = e(f"  sniffed out {total_versions} version(s) on Solana \u2014 this one's the OG!")
    header = (
        f"\U0001f43e * \\-Dooby\\-Doo\\! Found it\\!*\n"
...
async def build_leaderboard_text(scan_prices: dict, http: httpx.AsyncClient, bot_data: dict, chat_id: str = "") -> str:
    ...
    if not chat_entries:
        return (
            ...
            "Drop a `\\$TICKER` or CA here and   will start tracking\\!"
        )
...
_WATCH_CONFIRM_LINES = [
    r" 's on the case\! I'll sniff out every *{terms}* coin the moment one launches\!",
    r"Rooby\-Rooby\-Roo\!  's nose is tuned to *{terms}* — any new one and I'll bark\!",
    r" \-Dooby\-Doo\! 🔍  , I'm watching for *{terms}* tokens like a good boy\!",
    r"  You got it\! My super sniffer is locked onto *{terms}* — I won't miss a thing\!",
    r"  Leave it to  \! I'll howl the second I smell a new *{terms}* coin\!",
]

_WATCH_ALERT_INTROS = [
    r"Ruh\-roh\!   sniffed one out\!",
    r" \-Dooby\-Doo\! 🔍 Found a fresh one,  \!",
    r"  A brand new one just launched\!",
    r"   's nose never lies — new token detected\!",
    r"Rooby\-Rooby\-Roo\! Just launched and   caught it\!",
]
