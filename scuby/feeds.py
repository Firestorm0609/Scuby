...
async def scan_movers(
    ...
    Uses the shared pair_cache pool to avoid redundant API calls.
    ...
    # Use shared pair pool from pair_cache (90s TTL, ~60% fewer API calls)
    from pair_cache import get_solana_pairs
    all_pairs = await get_solana_pairs(http)
    logger.info(f"scan_movers: {len(all_pairs)} pairs from shared pool")
