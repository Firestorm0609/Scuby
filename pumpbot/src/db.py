...
async def update_ath_if_higher(mint: str, ath_mcap_sol: float, ath_ts):
    pool = await get_pool()
    await pool.execute(
        """
        UPDATE tokens
        SET max_mcap_sol = $2, max_mcap_at = $3
        WHERE mint = $1
          AND (max_mcap_sol IS NULL OR $2 > max_mcap_sol)
        """,
        mint, ath_mcap_sol, ath_ts,
    )
